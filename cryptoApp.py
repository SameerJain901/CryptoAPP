...
sstate=SessionState.get(api_key='',api_secret='',access_token='',access_secret='')

pages=['API Setup','Get Data','EDA','Analysis']
st.sidebar.title('Crypto Currency Sentiment Analysis')
out=st.sidebar.radio('Page:',pages)

if out=='API Setup':
    api_key=st.text_input("API Key",value=sstate.api_key)
    api_secret=st.text_input("API Secret",value=sstate.api_secret)
    access_token=st.text_input("Access Token",value=sstate.access_token)
    access_secret=st.text_input("Acess Secret",value=sstate.access_secret)
    submit=st.button('Submit')
    if submit:
        sstate.api_key=api_key
        sstate.api_secret=api_secret
        sstate.access_token=access_token
        sstate.access_secret=access_secret
        st.info('Got the required details')

elif out=='Get Data':
    try:
        
        api = connect_to_twitter_OAuth(sstate.api_key,sstate.api_secret,sstate.access_token,sstate.access_secret)
    except:
